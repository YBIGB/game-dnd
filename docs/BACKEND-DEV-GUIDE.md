...
```
GET  /api/shop/inventory          # 获取商店商品列表（含 NPC 资金）
POST /api/shop/trade              # 执行交易
```

| 任务 | 标识 | 状态 |
|------|------|------|
| 商店商品列表接口 | `[TRADE-01]` | 📝 |
| 交易执行接口（含金币校验） | `[TRADE-02]` | 📝 |
| NPC 资金扣减与校验逻辑 | `[TRADE-03]` | 📝 |

**关键点：**

| 项目 | 要求 |
|------|------|
| NPC 资金 | NPC 有自己的资金表（或角色表上扩展 npcGold 字段），与玩家金币相同机制 |
| 金币即物品 | 金币在物品表中以 id="gold" 标识，qty 即为持有数量，作为交易等价物 |
| 交易方向 | `direction: "buy"` 为玩家买（扣玩家金币），`"sell"` 为玩家卖（加玩家金币） |
| 购买校验 | `buy` 时检查 `playerGold >= item.value`，不足返回 code=5001 |
| 出售校验 | `sell` 时检查 `npcGold >= item.value`，不足返回 code=5001 |
| 物品扣减 | 交易成功后更新双方背包物品 qty，qty=0 时移除该物品 |
| 响应格式 | 与 `API-CONTRACT.md §5.2` 一致 |

**实现顺序建议：**

| 优先级 | 任务标识 | 说明 |
|--------|---------|------|
| 🔴 P0 | `[TRADE-01]` | 获取商店列表，前端 mock 已就绪 |
| 🟡 P1 | `[TRADE-02]`, `[TRADE-03]` | 交易执行 + 资金校验，后端必须严格校验余额 |
