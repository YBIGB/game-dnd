...
### Commit 规范
- 所有 commit message 统一使用**中文**描述
-  必须使用 conventional commit 前缀，如feat: / fix: / chore: / docs: / refactor: / test: 


### 阶段性收尾响应
当用户说「工作阶段性告一段落」时，自动执行以下流程：

1. **状态检查**：运行 git status 检查工作树状态（未跟踪文件、修改、暂存区）
2. **清理确认**：若有未提交改动，先 review 一遍改动内容是否合理；若有 lint/测试脚本可运行则运行
3. **更新 AGENTS.md**：如有新的开发约定、规则或项目状态变化需要记录，先更新本文件
4. **提交**：工作树有合理改动且无其他待办事项时，执行 git add && git commit
5. **汇报**：向用户简要报告提交内容和当前项目状态
